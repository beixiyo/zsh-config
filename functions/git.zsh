...
function gdiff() {
  ...
  # Preview: 有 diff 用 delta（无则 cat），无 diff 用 bat（无则 cat）
  local preview_cmd='
    file={3}
    ...
    if [[ -n "$diff" ]]; then
      ...
      command -v bat &>/dev/null && bat --color=always --style=numbers "$file" 2>/dev/null || cat "$file" 2>/dev/null || echo "No diff available for $file"
    fi'
...
function glog() {
  ...
  local preview_cmd='commit=$(echo {} | grep -o "[a-f0-9]\{7,40\}" | head -1); out=$(git show --color=always "$commit" 2>/dev/null); if command -v delta &>/dev/null; then echo "$out" | delta --width=${FZF_PREVIEW_COLUMNS:-80}; else echo "$out"; fi'
